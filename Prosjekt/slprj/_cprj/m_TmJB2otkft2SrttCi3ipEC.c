...
#include "m_TmJB2otkft2SrttCi3ipEC.h"
#include <string.h>
...
static emlrtRSInfo ab_emlrtRSI = { 118,/* lineNo */
  "carrierSyncCore",                   /* fcnName */
  "C:\\Program Files\\MATLAB\\R2024b\\toolbox\\comm\\comm\\+comm\\+internal\\+carrierSync\\carrierSyncCore.m"/* pathName */
};
...
static emlrtRSInfo eb_emlrtRSI = { 3,  /* lineNo */
  "",                                  /* fcnName */
  ""                                   /* pathName */
};
...
static void cgxe_mdl_start(InstanceStruct_TmJB2otkft2SrttCi3ipEC *moduleInstance);
static void cgxe_mdl_initialize(InstanceStruct_TmJB2otkft2SrttCi3ipEC
  *moduleInstance);
static void cgxe_mdl_outputs(InstanceStruct_TmJB2otkft2SrttCi3ipEC
  *moduleInstance);
static void cgxe_mdl_update(InstanceStruct_TmJB2otkft2SrttCi3ipEC
  *moduleInstance);
static void cgxe_mdl_derivative(InstanceStruct_TmJB2otkft2SrttCi3ipEC
  *moduleInstance);
static void cgxe_mdl_enable(InstanceStruct_TmJB2otkft2SrttCi3ipEC
  *moduleInstance);
static void cgxe_mdl_disable(InstanceStruct_TmJB2otkft2SrttCi3ipEC
  *moduleInstance);
static void cgxe_mdl_terminate(InstanceStruct_TmJB2otkft2SrttCi3ipEC
  *moduleInstance);
static void mw__internal__call__setup(InstanceStruct_TmJB2otkft2SrttCi3ipEC
...
static void CarrierSynchronizer_set_NormalizedLoopBandwidth(const emlrtStack *sp,
  comm_CarrierSynchronizer *obj, real_T b_value);
static void CarrierSynchronizer_validatePropertiesImpl(const emlrtStack *sp,
  comm_CarrierSynchronizer *obj);
...
static void SystemCore_checkTunablePropChange(const emlrtStack *sp,
  comm_CarrierSynchronizer *obj);
static void mw__internal__call__reset(InstanceStruct_TmJB2otkft2SrttCi3ipEC
  *moduleInstance, const emlrtStack *sp, real_T SamplesPerSymbol, real_T
  DampingFactor, real_T NormalizedLoopBandwidth);
static void mw__internal__call__step(InstanceStruct_TmJB2otkft2SrttCi3ipEC
  *moduleInstance, const emlrtStack *sp, real_T SamplesPerSymbol, real_T
  DampingFactor, real_T NormalizedLoopBandwidth, creal_T b_u0_data[], int32_T
  u0_size[2], creal_T b_y0_data[], int32_T y0_size[2]);
static void mw__internal__system___fcn(InstanceStruct_TmJB2otkft2SrttCi3ipEC
...
static const mxArray *emlrt_marshallOut(InstanceStruct_TmJB2otkft2SrttCi3ipEC
  *moduleInstance, const emlrtStack *sp);
static const mxArray *cgxe_mdl_get_sim_state
  (InstanceStruct_TmJB2otkft2SrttCi3ipEC *moduleInstance);
static void emlrt_marshallIn(InstanceStruct_TmJB2otkft2SrttCi3ipEC
...
static void cgxe_mdl_set_sim_state(InstanceStruct_TmJB2otkft2SrttCi3ipEC
...
static void s_emlrt_marshallIn(const emlrtStack *sp, const mxArray *src, const
  emlrtMsgIdentifier *msgId, coder_array_creal_T_2D *ret);
static void carrierSyncCore(const emlrtStack *sp, creal_T input_data[], int32_T
  ...
  *DDSPreviousInp, coder_array_creal_T_2D *prevHalfSymbol, real_T *phase, real_T
  *digitalSynthesizerGain, real_T *integratorGain, real_T *proportionalGain,
  ...
  phaseEstimate_data[], int32_T phaseEstimate_size[1], coder_array_creal_T_2D
  *prevHalfSymbolOut);
...
static void init_simulink_io_address(InstanceStruct_TmJB2otkft2SrttCi3ipEC
  *moduleInstance);
  ...
  *moduleInstance)
{
  ...
  creal_T u_tmp3_data[97];
  creal_T y_tmp0_data[97];
  real_T *DampingFactor;
  real_T *NormalizedLoopBandwidth;
  real_T *SamplesPerSymbol;
  ...
  DampingFactor, real_T NormalizedLoopBandwidth)
{
  ...
  static int8_T iv1[8] = { 97, 1, 1, 1, 1, 1, 1, 1 };
  ...
  CarrierSynchronizer_validatePropertiesImpl(&c_st, &moduleInstance->sysobj);
  b_st.site = &e_emlrtRSI;
  moduleInstance->sysobj.pPhase = 0.0;
  moduleInstance->sysobj.pPreviousSample.re = 0.0;
  moduleInstance->sysobj.pPreviousSample.im = 0.0;
  d = moduleInstance->sysobj.SamplesPerSymbol / 2.0 + 1.0;
  d = emlrtNonNegativeCheckR2012b(d, &b_emlrtDCI, &b_st);
  unnamed_idx_1 = (int32_T)emlrtIntegerCheckR2012b(d, &emlrtDCI, &b_st);
  array_creal_T_2D_SetSize(&moduleInstance->sysobj.pPreviousHalfSymbol, 1,
    unnamed_idx_1);
  for (i = 0; i < unnamed_idx_1; i++) {
    moduleInstance->sysobj.pPreviousHalfSymbol.vector.data[i].re = 0.0;
    moduleInstance->sysobj.pPreviousHalfSymbol.vector.data[i].im = 0.0;
  }
  ...
  comm_CarrierSynchronizer *obj)
{
  ...
  static char_T d_u[35] = { 'c', 'o', 'm', 'm', ':', 'C', 'a', 'r', 'r', 'i',
    'e', 'r', 'S', 'y', 'n', 'c', 'h', 'r', 'o', 'n', 'i', 'z', 'e', 'r', ':',
    'S', 'p', 's', 'N', 'o', 't', 'E', 'v', 'e', 'n' };

  static char_T c_u[19] = { 'M', 'A', 'T', 'L', 'A', 'B', ':', 'n', 'o', 'l',
    'o', 'g', 'i', 'c', 'a', 'l', 'n', 'a', 'n' };
  ...
  if (muDoubleScalarIsNaN(x)) {
    ...
    y = NULL;
    m = emlrtCreateCharArray(2, &iv[0]);
    emlrtInitCharArrayR2013a(&st, 19, m, &u[0]);
    emlrtAssign(&y, m);
    for (i = 0; i < 19; i++) {
      u[i] = c_u[i];
    }
    ...
    error(&b_st, y, getString(&b_st, c_message(&b_st, c_y, &i_emlrtMCI),
  ...
  if (x != 0.0) {
    ...
    b_y = NULL;
    m = emlrtCreateCharArray(2, &iv1[0]);
    emlrtInitCharArrayR2013a((emlrtConstCTX)sp, 35, m, &b_u[0]);
    emlrtAssign(&b_y, m);
    for (i = 0; i < 35; i++) {
      b_u[i] = d_u[i];
    }
    ...
    emlrtAssign(&d_y, m);
    st.site = &p_emlrtRSI;
    error(&st, b_y, getString(&st, c_message(&st, d_y, &h_emlrtMCI), &h_emlrtMCI),
          &h_emlrtMCI);
  }
}
  ...
  u0_size[2], creal_T b_y0_data[], int32_T y0_size[2])
{
  ...
  emlrtDimSizeGeqCheckR2012b(97, b_y0_size[0], &emlrtECI, (void *)sp);
  ...
  creal_T varargout_1_data[], int32_T varargout_1_size[1])
{
  ...
  cell_wrap varSizes[1];
  coder_array_creal_T_2D b_prevHalfSymbol;
  coder_array_creal_T_2D prevHalfSymbol;
  emlrtStack b_st;
  emlrtStack c_st;
  emlrtStack d_st;
  emlrtStack e_st;
  ...
  if (moduleInstance->sysobj.isInitialized != 1) {
    ...
    CarrierSynchronizer_validatePropertiesImpl(&e_st, &moduleInstance->sysobj);
    d_st.site = &e_emlrtRSI;
    moduleInstance->sysobj.pPhase = 0.0;
    moduleInstance->sysobj.pPreviousSample.re = 0.0;
    moduleInstance->sysobj.pPreviousSample.im = 0.0;
    PhaseRecoveryLoopBandwidth = moduleInstance->sysobj.SamplesPerSymbol / 2.0 +
      1.0;
    PhaseRecoveryLoopBandwidth = emlrtNonNegativeCheckR2012b
      (PhaseRecoveryLoopBandwidth, &b_emlrtDCI, &d_st);
    ret = (int32_T)emlrtIntegerCheckR2012b(PhaseRecoveryLoopBandwidth, &emlrtDCI,
      &d_st);
    array_creal_T_2D_SetSize(&moduleInstance->sysobj.pPreviousHalfSymbol, 1, ret);
    for (i = 0; i < ret; i++) {
      moduleInstance->sysobj.pPreviousHalfSymbol.vector.data[i].re = 0.0;
      moduleInstance->sysobj.pPreviousHalfSymbol.vector.data[i].im = 0.0;
    }
  ...
  if (moduleInstance->sysobj.TunablePropsChanged) {
    ...
    moduleInstance->sysobj.pIntegratorGain = 4.0 * PhaseRecoveryLoopBandwidth *
      PhaseRecoveryLoopBandwidth / d / (2.0 * PhaseRecoveryGain);
    for (i = 0; i < 4; i++) {
      a[i] = moduleInstance->sysobj.ModulationPhaseOffset[i];
    }

    for (i = 0; i < 4; i++) {
      b[i] = correctedValue[i];
    }
  ...
  b_st.site = &e_emlrtRSI;
  inSize[0] = (int8_T)varargin_7_size[0];
  inSize[1] = 1;
  for (i = 0; i < 6; i++) {
    inSize[i + 2] = 1;
  }
  ...
  b_st.site = &e_emlrtRSI;
  PhaseRecoveryLoopBandwidth = moduleInstance->sysobj.pLoopFilterState;
  d = moduleInstance->sysobj.pIntegFilterState;
  PhaseRecoveryGain = moduleInstance->sysobj.pDDSPreviousInput;
  previousSample.re = moduleInstance->sysobj.pPreviousSample.re;
  previousSample.im = moduleInstance->sysobj.pPreviousSample.im;
  array_creal_T_2D_Constructor(&prevHalfSymbol);
  array_creal_T_2D_SetSize(&prevHalfSymbol, 1,
    moduleInstance->sysobj.pPreviousHalfSymbol.size[1]);
  ret = moduleInstance->sysobj.pPreviousHalfSymbol.size[1];
  for (i = 0; i < ret; i++) {
    prevHalfSymbol.vector.data[i].re =
      moduleInstance->sysobj.pPreviousHalfSymbol.vector.data[i].re;
    prevHalfSymbol.vector.data[i].im =
      moduleInstance->sysobj.pPreviousHalfSymbol.vector.data[i].im;
  }

  phase = moduleInstance->sysobj.pPhase;
  digitalSynthesizerGain = moduleInstance->sysobj.pDigitalSynthesizerGain;
  integratorGain = moduleInstance->sysobj.pIntegratorGain;
  proportionalGain = moduleInstance->sysobj.pProportionalGain;
  phaseOffset = moduleInstance->sysobj.pActualPhaseOffset;
  array_creal_T_2D_Constructor(&b_prevHalfSymbol);
  c_st.site = &x_emlrtRSI;
  carrierSyncCore(&c_st, varargin_7_data, varargin_7_size,
                  &PhaseRecoveryLoopBandwidth, &d, &PhaseRecoveryGain,
                  &prevHalfSymbol, &phase, &digitalSynthesizerGain,
                  &integratorGain, &proportionalGain, &phaseOffset,
                  varargout_1_data, varargout_1_size, phaseEstimate_data,
                  phaseEstimate_size, &b_prevHalfSymbol);
  array_creal_T_2D_Destructor(&prevHalfSymbol);
  moduleInstance->sysobj.pLoopFilterState = PhaseRecoveryLoopBandwidth;
  moduleInstance->sysobj.pIntegFilterState = d;
  moduleInstance->sysobj.pDDSPreviousInput = PhaseRecoveryGain;
  moduleInstance->sysobj.pPreviousSample.re = previousSample.re;
  moduleInstance->sysobj.pPreviousSample.im = previousSample.im;
  array_creal_T_2D_SetSize(&moduleInstance->sysobj.pPreviousHalfSymbol, 1,
    b_prevHalfSymbol.size[1]);
  ret = b_prevHalfSymbol.size[1];
  for (i = 0; i < ret; i++) {
    moduleInstance->sysobj.pPreviousHalfSymbol.vector.data[i].re =
      b_prevHalfSymbol.vector.data[i].re;
    moduleInstance->sysobj.pPreviousHalfSymbol.vector.data[i].im =
      b_prevHalfSymbol.vector.data[i].im;
  }

  array_creal_T_2D_Destructor(&b_prevHalfSymbol);
  moduleInstance->sysobj.pPhase = phase;
  moduleInstance->sysobj.pDigitalSynthesizerGain = digitalSynthesizerGain;
  moduleInstance->sysobj.pIntegratorGain = integratorGain;
  moduleInstance->sysobj.pProportionalGain = proportionalGain;
  moduleInstance->sysobj.pActualPhaseOffset = phaseOffset;
  b_st.site = &e_emlrtRSI;
  SystemCore_checkTunablePropChange(&b_st, &moduleInstance->sysobj);
}
  ...
  *moduleInstance, const emlrtStack *sp)
{
  ...
  *(int32_T *)emlrtMxGetData(m) = u;
  ...
  for (i = 0; i < 4; i++) {
    e_u[i] = moduleInstance->sysobj.ModulationPhaseOffset[i];
  }
  ...
  array_creal_T_2D_Constructor(&g_u);
  array_creal_T_2D_SetSize(&g_u, 1,
    moduleInstance->sysobj.pPreviousHalfSymbol.size[1]);
  u = moduleInstance->sysobj.pPreviousHalfSymbol.size[1];
  for (i = 0; i < u; i++) {
    g_u.vector.data[i].re =
      moduleInstance->sysobj.pPreviousHalfSymbol.vector.data[i].re;
    g_u.vector.data[i].im =
      moduleInstance->sysobj.pPreviousHalfSymbol.vector.data[i].im;
  }
  ...
  m = emlrtCreateNumericArray(2, &iv3[0], mxDOUBLE_CLASS, mxCOMPLEX);
  emlrtExportNumericArrayR2013b((emlrtConstCTX)sp, m, &g_u.vector.data[0], 8);
  array_creal_T_2D_Destructor(&g_u);
  emlrtAssign(&v_y, m);
  ...
  emlrtMsgIdentifier *parentId, comm_CarrierSynchronizer *y)
{
  ...
  m = emlrtGetFieldR2017b((emlrtConstCTX)sp, u, 0, 13, "pPreviousSample");
  y->pPreviousSample = j_emlrt_marshallIn(sp, emlrtAlias(m), &thisId);
  ...
  k_emlrt_marshallIn(sp, emlrtAlias(emlrtGetFieldR2017b((emlrtConstCTX)sp, u, 0,
    18, "pPreviousHalfSymbol")), &thisId, &y->pPreviousHalfSymbol);
  emlrtDestroyArray(&u);
}
  ...
  emlrtMsgIdentifier *msgId, coder_array_creal_T_2D *ret)
{
  ...
  array_creal_T_2D_SetSize(ret, iv[0], iv[1]);
  emlrtImportArrayR2015b((emlrtConstCTX)sp, src, &ret->vector.data[0], 8, true);
  emlrtDestroyArray(&src);
}
...
static void carrierSyncCore(const emlrtStack *sp, creal_T input_data[], int32_T
  ...
  *prevHalfSymbolOut)
{
  coder_array_creal_T_2D prevHalfSymbolRot;
  coder_array_creal_T_2D tPrevHalfSymbol;
  emlrtStack b_st;
  emlrtStack st;
  creal_T phaseCorrection_data[97];
  real_T cosVal;
  real_T im;
  real_T loopFiltOut;
  real_T sinVal;
  ...
  array_creal_T_2D_Constructor(&tPrevHalfSymbol);
  array_creal_T_2D_SetSize(&tPrevHalfSymbol, 1, prevHalfSymbol->size[1]);
  loop_ub = prevHalfSymbol->size[1];
  for (i = 0; i < loop_ub; i++) {
    tPrevHalfSymbol.vector.data[i].re = prevHalfSymbol->vector.data[i].re;
    tPrevHalfSymbol.vector.data[i].im = prevHalfSymbol->vector.data[i].im;
  }

  i = input_size[0];
  array_creal_T_2D_Constructor(&prevHalfSymbolRot);
  for (k = 0; k < i; k++) {
    if (tPrevHalfSymbol.size[1] < 2) {
      i1 = 0;
      i2 = 0;
    } else {
      i1 = 1;
      i2 = emlrtDynamicBoundsCheckR2012b(tPrevHalfSymbol.size[1], 1,
        tPrevHalfSymbol.size[1], &emlrtBCI, (emlrtConstCTX)sp);
    }

    i3 = i2 - i1;
    loop_ub = i2 - i1;
    for (i4 = 0; i4 < loop_ub; i4++) {
      tPrevHalfSymbol.vector.data[i4].re = tPrevHalfSymbol.vector.data[i1 + i4].
        re;
      tPrevHalfSymbol.vector.data[i4].im = tPrevHalfSymbol.vector.data[i1 + i4].
        im;
    }

    array_creal_T_2D_SetSize(&tPrevHalfSymbol, 1, (i2 - i1) + 1);
    tPrevHalfSymbol.vector.data[i3].re =
      input_data[emlrtDynamicBoundsCheckR2012b(k + 1, 1, input_size[0],
      &c_emlrtBCI, (emlrtConstCTX)sp) - 1].re;
    tPrevHalfSymbol.vector.data[i3].im =
      input_data[emlrtDynamicBoundsCheckR2012b(k + 1, 1, input_size[0],
      &c_emlrtBCI, (emlrtConstCTX)sp) - 1].im;
    st.site = &y_emlrtRSI;
    sinVal = sin(*phase);
    cosVal = cos(*phase);
    array_creal_T_2D_SetSize(&prevHalfSymbolRot, 1, tPrevHalfSymbol.size[1]);
    loop_ub = tPrevHalfSymbol.size[1];
    for (i1 = 0; i1 < loop_ub; i1++) {
      loopFiltOut = tPrevHalfSymbol.vector.data[i1].re * cosVal -
        tPrevHalfSymbol.vector.data[i1].im * sinVal;
      im = tPrevHalfSymbol.vector.data[i1].re * sinVal +
        tPrevHalfSymbol.vector.data[i1].im * cosVal;
      prevHalfSymbolRot.vector.data[i1].re = loopFiltOut;
      prevHalfSymbolRot.vector.data[i1].im = im;
    }

    output_data[emlrtDynamicBoundsCheckR2012b(k + 1, 1, output_size[0],
      &e_emlrtBCI, (emlrtConstCTX)sp) - 1].re =
      prevHalfSymbolRot.vector.data[emlrtDynamicBoundsCheckR2012b
      (prevHalfSymbolRot.size[1], 1, prevHalfSymbolRot.size[1], &d_emlrtBCI,
       (emlrtConstCTX)sp) - 1].re;
    output_data[emlrtDynamicBoundsCheckR2012b(k + 1, 1, output_size[0],
      &e_emlrtBCI, (emlrtConstCTX)sp) - 1].im =
      prevHalfSymbolRot.vector.data[emlrtDynamicBoundsCheckR2012b
      (prevHalfSymbolRot.size[1], 1, prevHalfSymbolRot.size[1], &d_emlrtBCI,
       (emlrtConstCTX)sp) - 1].im;
    st.site = &ab_emlrtRSI;
    b_st.site = &db_emlrtRSI;
    loopFiltOut = signDouble(prevHalfSymbolRot.vector.data[0].re);
    st.site = &bb_emlrtRSI;
    emlrtDynamicBoundsCheckR2012b(prevHalfSymbolRot.size[1], 1,
      prevHalfSymbolRot.size[1], &b_emlrtBCI, &st);
    b_st.site = &db_emlrtRSI;
    sinVal = signDouble(prevHalfSymbolRot.vector.data[prevHalfSymbolRot.size[1]
                        - 1].im);
    cosVal = loopFiltOut * prevHalfSymbolRot.vector.data[0].im - sinVal *
      prevHalfSymbolRot.vector.data[emlrtDynamicBoundsCheckR2012b
      (prevHalfSymbolRot.size[1], 1, prevHalfSymbolRot.size[1], &f_emlrtBCI,
       (emlrtConstCTX)sp) - 1].re;
    loopFiltOut = cosVal * *integratorGain + *loopFiltState;
    *loopFiltState = loopFiltOut;
    sinVal = *DDSPreviousInp + *integFiltState;
    *integFiltState = sinVal;
    *DDSPreviousInp = cosVal * *proportionalGain + loopFiltOut;
    *phase = *digitalSynthesizerGain * sinVal;
    phaseCorrection_data[emlrtDynamicBoundsCheckR2012b(k + 1, 1,
      phaseCorrection_size_idx_0, &g_emlrtBCI, (emlrtConstCTX)sp) - 1].re =
      *phase;
    phaseCorrection_data[emlrtDynamicBoundsCheckR2012b(k + 1, 1,
      phaseCorrection_size_idx_0, &g_emlrtBCI, (emlrtConstCTX)sp) - 1].im = 0.0;
  }

  array_creal_T_2D_Destructor(&prevHalfSymbolRot);
  array_creal_T_2D_SetSize(prevHalfSymbolOut, 1, tPrevHalfSymbol.size[1]);
  loop_ub = tPrevHalfSymbol.size[1];
  for (i = 0; i < loop_ub; i++) {
    prevHalfSymbolOut->vector.data[i].re = tPrevHalfSymbol.vector.data[i].re;
    prevHalfSymbolOut->vector.data[i].im = tPrevHalfSymbol.vector.data[i].im;
  }

  array_creal_T_2D_Destructor(&tPrevHalfSymbol);
  st.site = &cb_emlrtRSI;
  sinVal = sin(*phaseOffset);
  cosVal = cos(*phaseOffset);
  loop_ub = output_size[0];
  phaseEstimate_size[0] = input_size[0];
  for (i = 0; i < loop_ub; i++) {
    im = output_data[i].re * sinVal + output_data[i].im * cosVal;
    output_data[i].re = output_data[i].re * cosVal - output_data[i].im * sinVal;
    output_data[i].im = im;
  ...
  *moduleInstance)
{
  moduleInstance->emlrtRootTLSGlobal = (void *)cgxertGetEMLRTCtx
    (moduleInstance->S);
  moduleInstance->u0_data = (creal_T (*)[97])cgxertGetInputPortSignal
    (moduleInstance->S, 0);
  moduleInstance->u0_sizes = (int32_T (*)[2])cgxertGetCurrentInputPortDimensions
    (moduleInstance->S, 0);
  moduleInstance->y0_data = (creal_T (*)[97])cgxertGetOutputPortSignal
    (moduleInstance->S, 0);
...
/* CGXE Glue Code */
static void mdlOutputs_TmJB2otkft2SrttCi3ipEC(SimStruct *S, int_T tid)
{
  InstanceStruct_TmJB2otkft2SrttCi3ipEC *moduleInstance =
    (InstanceStruct_TmJB2otkft2SrttCi3ipEC *)cgxertGetRuntimeInstance(S);
  cgxe_mdl_outputs(moduleInstance);
}

static void mdlInitialize_TmJB2otkft2SrttCi3ipEC(SimStruct *S)
{
  InstanceStruct_TmJB2otkft2SrttCi3ipEC *moduleInstance =
    (InstanceStruct_TmJB2otkft2SrttCi3ipEC *)cgxertGetRuntimeInstance(S);
...
static void mdlUpdate_TmJB2otkft2SrttCi3ipEC(SimStruct *S, int_T tid)
{
  InstanceStruct_TmJB2otkft2SrttCi3ipEC *moduleInstance =
    (InstanceStruct_TmJB2otkft2SrttCi3ipEC *)cgxertGetRuntimeInstance(S);
...
static void mdlDerivatives_TmJB2otkft2SrttCi3ipEC(SimStruct *S)
{
  InstanceStruct_TmJB2otkft2SrttCi3ipEC *moduleInstance =
    (InstanceStruct_TmJB2otkft2SrttCi3ipEC *)cgxertGetRuntimeInstance(S);
...
static mxArray* getSimState_TmJB2otkft2SrttCi3ipEC(SimStruct *S)
{
  mxArray* mxSS;
  InstanceStruct_TmJB2otkft2SrttCi3ipEC *moduleInstance =
    (InstanceStruct_TmJB2otkft2SrttCi3ipEC *)cgxertGetRuntimeInstance(S);
...
static void setSimState_TmJB2otkft2SrttCi3ipEC(SimStruct *S, const mxArray *ss)
{
  InstanceStruct_TmJB2otkft2SrttCi3ipEC *moduleInstance =
    (InstanceStruct_TmJB2otkft2SrttCi3ipEC *)cgxertGetRuntimeInstance(S);
...
static void mdlTerminate_TmJB2otkft2SrttCi3ipEC(SimStruct *S)
{
  InstanceStruct_TmJB2otkft2SrttCi3ipEC *moduleInstance =
    (InstanceStruct_TmJB2otkft2SrttCi3ipEC *)cgxertGetRuntimeInstance(S);
...
static void mdlEnable_TmJB2otkft2SrttCi3ipEC(SimStruct *S)
{
  InstanceStruct_TmJB2otkft2SrttCi3ipEC *moduleInstance =
    (InstanceStruct_TmJB2otkft2SrttCi3ipEC *)cgxertGetRuntimeInstance(S);
...
static void mdlDisable_TmJB2otkft2SrttCi3ipEC(SimStruct *S)
{
  InstanceStruct_TmJB2otkft2SrttCi3ipEC *moduleInstance =
    (InstanceStruct_TmJB2otkft2SrttCi3ipEC *)cgxertGetRuntimeInstance(S);
...
static void mdlStart_TmJB2otkft2SrttCi3ipEC(SimStruct *S)
{
  InstanceStruct_TmJB2otkft2SrttCi3ipEC *moduleInstance =
    (InstanceStruct_TmJB2otkft2SrttCi3ipEC *)calloc(1, sizeof
    (InstanceStruct_TmJB2otkft2SrttCi3ipEC));
  moduleInstance->S = S;
  cgxertSetRuntimeInstance(S, (void *)moduleInstance);
  ssSetmdlOutputs(S, mdlOutputs_TmJB2otkft2SrttCi3ipEC);
  ssSetmdlInitializeConditions(S, mdlInitialize_TmJB2otkft2SrttCi3ipEC);
  ssSetmdlUpdate(S, mdlUpdate_TmJB2otkft2SrttCi3ipEC);
  ssSetmdlDerivatives(S, mdlDerivatives_TmJB2otkft2SrttCi3ipEC);
  ssSetmdlTerminate(S, mdlTerminate_TmJB2otkft2SrttCi3ipEC);
  ssSetmdlEnable(S, mdlEnable_TmJB2otkft2SrttCi3ipEC);
  ssSetmdlDisable(S, mdlDisable_TmJB2otkft2SrttCi3ipEC);
...
static void mdlProcessParameters_TmJB2otkft2SrttCi3ipEC(SimStruct *S)
{
}

void method_dispatcher_TmJB2otkft2SrttCi3ipEC(SimStruct *S, int_T method, void
  *data)
{
  switch (method) {
   case SS_CALL_MDL_START:
    mdlStart_TmJB2otkft2SrttCi3ipEC(S);
    break;

   case SS_CALL_MDL_PROCESS_PARAMETERS:
    mdlProcessParameters_TmJB2otkft2SrttCi3ipEC(S);
    break;

   case SS_CALL_MDL_GET_SIM_STATE:
    *((mxArray**) data) = getSimState_TmJB2otkft2SrttCi3ipEC(S);
    break;

   case SS_CALL_MDL_SET_SIM_STATE:
    setSimState_TmJB2otkft2SrttCi3ipEC(S, (const mxArray *) data);
    break;
   ...
   default:
    ...
    /*
       ...
       "Error calling method dispatcher for module: TmJB2otkft2SrttCi3ipEC.\n"
...
mxArray *cgxe_TmJB2otkft2SrttCi3ipEC_BuildInfoUpdate(void)
{
  mxArray * mxBIArgs;
  mxArray * elem_1;
  mxArray * elem_2;
...
mxArray *cgxe_TmJB2otkft2SrttCi3ipEC_fallback_info(void)
{
